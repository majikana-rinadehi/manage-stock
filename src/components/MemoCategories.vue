...
<template>
    ...
    <div class="bg-gray-200 p-2 text-sm m-auto" style="min-width: 420px;">
        <div id="capture-items-target"
            class="bg-gray-200 p-2 text-sm m-auto">
            <!--カテゴリアイテム-->
            <div class="font-black text-lg">
                <!--カテゴリ名-->
            </div>
            <div
                class="flex justify-end items-center m-2 bg-white p-2"
                v-for="(item, index) in displayItems"
                v-bind:key="index">
                <!--アイテム-->
                <div class="bg-red-500 -mt-3 h-9 w-2 mr-3" v-if="item.period == 1"></div>
                <div class="bg-yellow-500 -mt-3 h-9 w-2 mr-3" v-else-if="item.period == 2"></div>
                <div class="bg-green-500 -mt-3 h-9 w-2 mr-3" v-else-if="item.period >= 3"></div>
                <div v-else class="-mt-3 h-7 w-2 mr-3"></div>
                ...
                <div class="mr-3">
                    <span>買う個数:</span>
                    <button class="font-bold" @click="increment(item)">
                        <!--プラスボタン-->
                        ＋
                    </button>
                    <span
                        class="border rounded-lg px-4 py-2 text-xs"
                    >{{ item.value }}{{ item.unit_name }}</span>
                    <button class="font-black" @click="decrement(item)">
                        <!--マイナスボタン-->
                        −
                    </button>
                </div>
                <div class="mr-0">
                    <button
                        class="px-4 py-2 bg-red-500 hover:bg-red-700 text-white rounded-lg font-bold text-xs ml-5"
        ...
        <ItemAdd_2 @item-added="addMemoItem" />
    </div>
</template>
